...
def _confidence_bar(conf: float, colour: str) -> str:
    ...
    return (
        ...
        f"<span style='font-family:Share Tech Mono,monospace;font-size:0.6rem;color:{colour};min-width:36px;'>{pct}%</span>"
        f"</div>"
    )
...
def _signal_card(sig: dict, node_colour: str) -> str:
    ...
    if sig["contraindications"]:
        ...
        contra_html = (
            ...
            f"<ul style='margin:0;padding-left:16px;font-size:0.65rem;'>{items}</ul>"
            f"</div>"
        )
    ...
    if sig.get("sources_au"):
        ...
        sources_html = (
            ...
            f"<ul style='margin:0;padding-left:14px;font-size:0.62rem;color:#aaa;'>{src_items}</ul>"
            f"</div>"
        )
    ...
    if sig.get("price_range_aud"):
        price_html = (
            ...
            f"font-size:0.6rem;color:#FFC94A;'>◈ {html.escape(sig['price_range_aud'])}</div>"
        )
...
<div style='margin-bottom:14px;padding:14px;
  ...
  <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;'>
    ...
    <span style='font-size:0.55rem;padding:2px 7px;border-radius:3px;
        background:{risk_col}22;color:{risk_col};border:1px solid {risk_col}55;
        white-space:nowrap;'>{html.escape(sig['risk_level'])}</span>
  </div>
  <div style='font-size:0.65rem;color:#888;font-style:italic;margin-bottom:8px;'>{mechanism}</div>
  <div style='display:flex;align-items:center;gap:8px;margin-bottom:6px;'>
    ...
    <span style='font-size:0.55rem;color:#555;'>{sig['freshness']} · DRIFT: {sig['drift']}</span>
  </div>
  {conf_bar}
  <div style='margin-top:10px;'>
    <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;
        color:#7DF4FF;letter-spacing:1px;margin-bottom:3px;'>ESTABLISHED</div>
    <div style='font-size:0.65rem;color:#ccc;line-height:1.5;'>{established}</div>
  </div>
  <div style='margin-top:8px;'>
    <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;
        color:#FFC94A;letter-spacing:1px;margin-bottom:3px;'>EMERGING</div>
    <div style='font-size:0.65rem;color:#aaa;font-style:italic;line-height:1.5;'>{emerging}</div>
  </div>
  <div style='margin-top:8px;'>
    <div style='font-family:Share Tech Mono,monospace;font-size:0.6rem;
        color:#555;letter-spacing:1px;margin-bottom:3px;'>KNOWN UNKNOWNS</div>
    <div style='font-size:0.62rem;color:#666;line-height:1.5;'>{html.escape(sig['known_unknowns'][:180])}</div>
  </div>
  {contra_html}
  {sources_html}
  {price_html}
...
<div style='padding:12px 16px;border:1px solid {primary}44;border-radius:10px;
  ...
  <div style='flex:1;'>
    ...
    <div style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:#555;
...
<div style='padding:10px 14px;border:1px solid #FF2E6355;border-radius:8px;
  ...
  <div style='font-size:0.65rem;color:#888;line-height:1.5;'>
...
""", unsafe_allow_html=True)
        for w in warnings:
            st.markdown(f"<div style='font-size:0.65rem;color:#FF6B6B;margin-bottom:4px;'>{html.escape(w)}</div>",
                        unsafe_allow_html=True)
        st.markdown("""
  <div style='font-size:0.62rem;color:#888;margin-top:8px;'>
...
<div style='margin-bottom:8px;padding:8px 14px;border-left:3px solid {colour};
  ...
  <div style='font-size:0.6rem;color:#666;margin-top:2px;'>{desc}</div>
</div>""", unsafe_allow_html=True)
        for sig in signals:
            risk_col = RISK_COLOURS.get(sig["risk_level"], "#888")
            st.markdown(
                f"<div style='padding:6px 12px;margin-bottom:4px;font-size:0.65rem;"
