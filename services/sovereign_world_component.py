...
def _fetch_state() -> dict:
    ...
    try:
        ...
        _live_truth = read_live_wallet_truth(DB_PATH, max_age_sec=180.0)
        wallet["live_wallet"] = float(_live_truth.get("balance_usd") or 0.0)
    except Exception:
        wallet["live_wallet"] = float(cfg("LIVE_WALLET_USD", 0) or 0)
    # Baseline remains accounting metadata, never a second wallet.
