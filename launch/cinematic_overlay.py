...
# ── SIGNOFF_MARK_HONESTY_20260812 ────────────────────────────────────────────
# This module previously derived PnL% itself with
#     last = float(pos.get("last_price") or entry)
#     pnl_pct = ((last - entry) / entry * 100) if entry > 0 else 0
# `0.0 or entry` evaluates to `entry`, so a position whose executable mark was
# genuinely worthless rendered as a flat 0.00% — a real rug became invisible —
# and a position with no mark at all rendered identically. The exit-gear badge
# below was then derived from that fabricated number, so an unpriced position
# could display "BASE · tp/sl/stagnation watch" while actually sitting past the
# hard stop.
#
# services/position_truth_payload.py (contract 2.0.0) is the canonical answer:
# it never substitutes, and returns hero_pnl_pct=None when no stage of evidence
# can carry authority. This module now consumes it and renders "NO MARK" rather
# than inventing a coordinate. If the payload module is unavailable we still do
# not fabricate — every row degrades to NO MARK.
def _truth_payloads(limit: int = 5) -> dict:
    """position_id -> canonical truth payload. Read-only, never raises."""
    try:
        from services.position_truth_payload import build_open_position_truth_payloads
    except Exception:
        return {}
    try:
        payloads = build_open_position_truth_payloads(
            matrix_db=str(DB_PATH), limit=int(limit))
        return {int(p.get("position_id") or -1): p for p in payloads}
    except Exception:
        return {}
...
def render_lifecycle_visual() -> None:
    try:
        ...
        for pos in positions:
            ...
            # Canonical mark. None means "we do not know", NOT "unchanged".
            tp = truth.get(int(pos.get("id") or -1)) or {}
            pnl_pct = tp.get("hero_pnl_pct")
            mark_price = tp.get("hero_price")
            pnl_usd = tp.get("hero_pnl_usd")
            priced = pnl_pct is not None
            ...
            if not priced:
                # No authoritative mark: the governor's own inputs are absent,
                # so no gear can be claimed. Showing "BASE" here would assert
                # that the position is calmly inside its thresholds when in
                # fact nothing has been evaluated.
                gear, gear_fn, gear_col = "UNPRICED", "awaiting executable mark", "#8A8FA3"
            elif pnl_pct <= -_gcfg["hard_stop"]:
            ...
            # Absent evidence renders as absent, in a neutral tone. It is not
            # green, because it is not a gain; it is not red, because it is not
            # a loss; it is grey, because it is not known.
            if priced:
                pnl_col = "#14F195" if (pnl_usd or 0) >= 0 else "#FF4444"
                pnl_usd_str = f"{'+' if (pnl_usd or 0) >= 0 else ''}{(pnl_usd or 0):.2f}"
            ...
            trail_str = ""
            if trail and mark_price:
                t = float(trail)
                m = float(mark_price)
                if t > 0 and m > 0:
                    trail_pct = ((m - t) / m * 100)
                    trail_str = f"trail {trail_pct:.1f}%"
            elif trail and not priced:
                trail_str = "trail —"
            ...
            st.markdown(
                ...
                <div style="display:flex;align-items:center;gap:10px;
                  ...
                  <span style="font-size:10px;color:{pnl_col};min-width:60px;">
                    {pnl_usd_str}</span>
                  <span style="font-size:9px;color:{pnl_col};">{pnl_pct_str}</span>
