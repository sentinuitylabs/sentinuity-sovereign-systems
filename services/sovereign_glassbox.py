...
def _svc_pill(name:str, age_s:float|None, note:str="") -> str:
    ...
    pulse_cls = ' gbx-alive' if (age_s is not None and age_s < 40) else ''
...
def _panel_pulse() -> dict:
    ...
    pills = "".join(
        _svc_pill(r["service_name"], r.get("age_s"), str(r.get("note") or ""))
        for r in hb["rows"])
...
def render_glassbox() -> None:
    ...
    try:
        ...
        st.caption(f"diagnostic console unavailable: {_dr_err}")
