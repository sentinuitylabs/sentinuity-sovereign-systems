...
# ── TIER 1 GEOMETRY (SIGNOFF_TIER1_DOCTRINE_20260716) ───────────────────────
# Root cause of the historical dead margins: width:100% + viewBox 680×344 +
# preserveAspectRatio="meet" + a CSS max-height clamp. Once the clamp engaged,
# "meet" scaled the drawing to fit the clamped height and CENTERED it — the
# wider the screen, the wider the dead margins. The correct fix is to make the
# WRAPPER's aspect-ratio equal the viewBox ratio (CSS aspect-ratio), so the SVG
# always fills its box exactly: no dead margins at any width, no distortion
# (preserveAspectRatio stays "meet" and simply never has slack to distribute).
# Mobile gets its OWN plot geometry (fewer labels, tighter pads, larger
# viewBox-relative type) — not a scaled-down desktop drawing.
_GEOM_DESKTOP = dict(W=1080, pad_l=52, pad_r=22, top=74, floor=254, H=300,
                     label_slots=16, fs_title=13, fs_sub=11, fs_net=12,
                     fs_axis=11, fs_legend=11, bar_min=3.0)
_GEOM_MOBILE  = dict(W=420,  pad_l=38, pad_r=12, top=64, floor=214, H=252,
                     label_slots=5,  fs_title=12, fs_sub=10, fs_net=11,
                     fs_axis=11, fs_legend=10, bar_min=2.5)


def _cadence_svg(buckets, labels, bucket_min, window_h, meta=None, geom=None):
    """Pure SVG renderer — green glass profit / red+violet glass loss.
    Data queries, bucketing, PnL math, labels and truth strips are unchanged;
    only plot geometry is parameterised (desktop wide / dedicated mobile)."""
    meta = meta or {}
    g = geom or _GEOM_DESKTOP
    W = g["W"]; H = g["H"]; pad_l = g["pad_l"]; pad_r = g["pad_r"]
    top = g["top"]; floor = g["floor"]
    plot_w = W - pad_l - pad_r
    n = len(buckets) or 1
    bw = plot_w / n
    barw = max(g["bar_min"], bw * 0.66)
    ...
    step = max(1, n // g["label_slots"])
    xlabels = []
    for i in range(0, n, step):
        cxx = pad_l + i * bw + bw / 2
        xlabels.append(f"<text x='{cxx:.0f}' y='{floor+15:.0f}' fill='{_C_AXIS}' "
                       f"font-family='monospace' font-size='{g['fs_axis']}' text-anchor='middle'>{labels[i]}</text>")
    ...
    # Sparse honesty: if activity occupies a small fraction of the window,
    # say so — never let a few bars visually imply a full timeline.
    occupied = sum(1 for v in buckets if abs(v) > 1e-9)
    sparse_txt = (f" · SPARSE — {occupied}/{n} buckets active"
                  if trades > 0 and n >= 8 and occupied / n < 0.15 else "")
...
<text x="{pad_l}" y="32" fill="{_C_CYAN}" font-family="'Share Tech Mono',monospace" font-size="{g['fs_title']}" letter-spacing="2">CLUSTER CADENCE — REALIZED PnL</text>
<text x="{W-pad_r}" y="32" fill="{_C_AXIS}" font-family="monospace" font-size="{g['fs_sub']}" text-anchor="end">{bucket_min}m · {window_h}h{sparse_txt}</text>
<text x="{pad_l}" y="48" fill="{net_col}" font-family="monospace" font-size="{g['fs_net']}">net {net_txt} · {trades} closed · {html.escape(str(source))}</text>
<line x1="{pad_l}" y1="{zero_y:.0f}" x2="{W-pad_r}" y2="{zero_y:.0f}" stroke="{_C_GRID}" stroke-width="0.5" stroke-dasharray="3 4"/>
<text x="{pad_l-6}" y="{zero_y+3:.0f}" fill="{_C_AXIS}" font-family="monospace" font-size="{g['fs_axis']}" text-anchor="end">$0</text>
{''.join(bars)}
{''.join(xlabels)}
<rect x="{pad_l}" y="{H+18}" width="13" height="13" rx="3" fill="url(#cadGreen)"/>
<text x="{pad_l+20}" y="{H+28}" fill="#9DB5A8" font-family="'Share Tech Mono',monospace" font-size="{g['fs_legend']}">PROFIT</text>
<rect x="{pad_l+96}" y="{H+18}" width="13" height="13" rx="3" fill="url(#cadRed)"/>
<text x="{pad_l+116}" y="{H+28}" fill="#C99" font-family="'Share Tech Mono',monospace" font-size="{g['fs_legend']}">LOSS — red w/ violet depth</text>
</svg>"""
...
def _diagnostics_html(meta):
    ...
    return ("<div style='font-family:monospace;font-size:0.66rem;color:#3a4754;"
...
def render_glass_cadence(db_path, table="paper_positions", key_prefix="sol",
    ...
    # SIGNOFF_TIER1_DOCTRINE_20260716: max-height clamps REMOVED — they were the
    # dead-margin mechanism (clamped height + "meet" + xMid = centered drawing
    # with blank flanks). Each variant's WRAPPER carries a CSS aspect-ratio
    # equal to its own viewBox ratio, so the SVG fills its box exactly at every
    # width: usable bounds match the neighbouring full-width sections, no
    # horizontal overflow, no distortion. Desktop and mobile are separate
    # drawings with their own plot geometry, toggled by media query — the
    # mobile chart is a different plot, not a shrunken desktop one.
    _dg, _mg = _GEOM_DESKTOP, _GEOM_MOBILE
    st.markdown(
        "<style>"
        ".sw-cadence-wrap{width:100%;}"
        f".sw-cadence-desktop{{aspect-ratio:{_dg['W']}/{_dg['H']+44};}}"
        f".sw-cadence-mobile{{aspect-ratio:{_mg['W']}/{_mg['H']+44};display:none;}}"
        ".sw-cadence-wrap svg{width:100%;height:100%;display:block;}"
        "@media (max-width:760px){"
        ".sw-cadence-desktop{display:none;}"
        ".sw-cadence-mobile{display:block;}}"
        "</style>",
    ...
    if meta["trades"] == 0 and meta.get("rows_loaded", 0) == 0:
        ...
        return
    st.markdown(
        "<div class='sw-cadence-wrap sw-cadence-desktop'>"
        + _cadence_svg(buckets, labels, bmin, wh, meta, _GEOM_DESKTOP)
        + "</div><div class='sw-cadence-wrap sw-cadence-mobile'>"
        + _cadence_svg(buckets, labels, bmin, wh, meta, _GEOM_MOBILE)
...
if __name__ == "__main__":
    ...
    assert f'viewBox="0 0 {_GEOM_DESKTOP["W"]} {_GEOM_DESKTOP["H"]+44}"' in svg
    assert "CLUSTER CADENCE — REALIZED PnL" in svg          # title stays truthful
    msvg = _cadence_svg(bk, lbls, 10, 4, {"net": sum(bk), "trades": len(bk)}, _GEOM_MOBILE)
    assert f'viewBox="0 0 {_GEOM_MOBILE["W"]} {_GEOM_MOBILE["H"]+44}"' in msvg
    assert msvg.count("text-anchor='middle'") <= _GEOM_MOBILE["label_slots"] + 1, \
        "mobile must carry fewer x labels, not shrunken desktop ones"
    # sparse honesty: 2 active of 24 buckets must be labelled sparse
    sparse_bk = [0.0] * 24; sparse_bk[3] = 12.0; sparse_bk[17] = -4.0
    ssvg = _cadence_svg(sparse_bk, lbls, 10, 4, {"net": 8.0, "trades": 2})
    assert "SPARSE — 2/24 buckets active" in ssvg
    # physical label size: 11 viewBox-units at 380px-wide mobile ≈ 10.0px ≥ readable;
    # desktop 11 units at 1100px real width ≈ 11.2px
    assert _GEOM_MOBILE["fs_axis"] >= 10 and _GEOM_DESKTOP["fs_axis"] >= 10
