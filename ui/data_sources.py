...
def heartbeats() -> dict:
    ...
    try:
        now = time.time()
        from ui.theme import service_heartbeat_thresholds, service_heartbeat_class
        rows = []
        for r in con.execute(
                "SELECT service_name, status, last_pulse, note "
                "FROM system_heartbeat ORDER BY service_name"):
            item = dict(r)
            age = now - float(item.get("last_pulse") or 0)
            fresh, aging = service_heartbeat_thresholds(item.get("service_name"))
            item.update({
                "age_s": age,
                "fresh_sec": fresh,
                "aging_sec": aging,
                "freshness_class": service_heartbeat_class(item.get("service_name"), age),
            })
            rows.append(item)
...
def latch_state() -> dict:
    ...
    try:
        vis = con.execute(
            ...
            "WHERE COALESCE(latched,0)=1 AND COALESCE(execution_ready,0)=1"
        ).fetchone()[0]
        jam = con.execute(
            "SELECT COUNT(*) FROM market_snapshots "
            "WHERE (COALESCE(latched,0)=1 OR COALESCE(execution_ready,0)=1) "
