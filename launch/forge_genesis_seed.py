...
Restores the standing-task contract and keeps copytrade influence in SHADOW measurement mode. Live remains observe-only because copytrade_influence enforces a
...
def _set_paper_copytrade_config() -> None:
    ...
    try:
        ...
        con.execute("INSERT INTO system_config(key,value) VALUES('COPYTRADE_PAPER_BONUS_ENABLED','0') ON CONFLICT(key) DO UPDATE SET value='0'")
        con.execute("INSERT INTO system_config(key,value) VALUES('SWTI_ENABLED','1') ON CONFLICT(key) DO UPDATE SET value='0'")
        settings = {
            'GMGN_WALLET_REFRESH_ENABLED': '1',
            'COPYTRADE_PAPER_BONUS_ENABLED': '0',
        ...
        con.commit()
        print("[FORGE] smart-wallet influence remains SHADOW-only; paper/live bonus disabled")
    finally:
        con.close()
