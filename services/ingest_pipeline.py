...
def run_pipeline() -> None:
    ...
    while True:
        ...
        try:
            ...
            if _is_db_lock(exc):
                lock_seen = True
                log.warning("[VALIDATE_DB_LOCK] yielding cycle: %s", exc)
            else:
                log.error("validate lane failed: %s: %s", type(exc).__name__, exc)
                try:
        ...
        try:
            ...
            if _is_db_lock(exc):
                lock_seen = True
                log.warning("[RESOLVER_DB_LOCK] yielding cycle: %s", exc)
            else:
                log.error("resolver lane failed: %s: %s", type(exc).__name__, exc)
                try:
        ...
        try:
            ...
            if _is_db_lock(exc):
                lock_seen = True
                log.warning("[WEAVER_DB_LOCK] yielding cycle: %s", exc)
            else:
                log.error("weaver lane failed: %s: %s", type(exc).__name__, exc)
                try:
        ...
        try:
            update_heartbeat(
                SERVICE_NAME,
                "DEGRADED" if lock_seen else "ALIVE",
                ("db_lock_backoff | " if lock_seen else "") + summary,
