...
def render_living_cortex():
    ...
    <div class="snty-crystal-panel snty-cyan-panel" style="padding:14px 18px;margin:10px 0 16px 0;">
      <div class="snty-title-row">
        <div class="snty-title-left">
          {_snty_crystalline_mark_svg(28, _exec_lbl not in ("BLOCKED","OFF"), "identity")}<span class="snty-section-title cyan">SYSTEM IDENTITY</span>
...
def _snty_crystalline_mark_svg(size: int = 42, verified: bool = True, uid: str = "hero") -> str:
    """Dependency-free Sentinuity identity mark with instance-safe SVG IDs."""
    safe_uid = re.sub(r"[^a-zA-Z0-9_-]", "", str(uid)) or "snty"
    close = '<path d="M31 14 L34 17 L37 14" />' if verified else '<path d="M31 14 L33 16" opacity=".52"/>'
    gold_core = (
        f'<path d="M34 5 L40 11 L34 17 L28 11 Z" fill="url(#gold-{safe_uid})" opacity=".10"/>'
        if verified else ''
    )
    return f"""
<svg class="snty-brand-mark" width="{int(size)}" height="{int(size)}" viewBox="0 0 68 68"
 fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Sentinuity identity">
 <defs>
  <linearGradient id="body-{safe_uid}" x1="8" y1="61" x2="39" y2="5" gradientUnits="userSpaceOnUse">
   <stop offset="0" stop-color="#14F195"/><stop offset=".30" stop-color="#8EF9FF"/>
   <stop offset=".58" stop-color="#9945FF"/><stop offset=".82" stop-color="#6D5BFF"/>
   <stop offset="1" stop-color="#FFD700"/>
  </linearGradient>
  <linearGradient id="gold-{safe_uid}" x1="28" y1="17" x2="40" y2="5" gradientUnits="userSpaceOnUse">
   <stop stop-color="#9945FF" stop-opacity=".18"/><stop offset="1" stop-color="#FFD700" stop-opacity=".72"/>
  </linearGradient>
  <filter id="glow-{safe_uid}" x="-50%" y="-50%" width="200%" height="200%">
   <feGaussianBlur stdDeviation=".65" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
  </filter>
 </defs>
 <g stroke="url(#body-{safe_uid})" stroke-width="2.25" stroke-linecap="round"
  stroke-linejoin="round" filter="url(#glow-{safe_uid})">
  <path d="M5 60 C13 55 15 45 22 42 C28 39 31 31 34 18"/>
  <path d="M12 64 C17 57 20 52 27 49 C31 47 33 38 34 30" opacity=".66"/>
  <path d="M61 58 C53 53 49 47 44 43 C39 39 37 29 34 18" opacity=".78"/>
  <path d="M19 58 C24 54 28 54 31 50" opacity=".24"/>
  <circle cx="22" cy="42" r="2.2"/><circle cx="44" cy="43" r="2.2"/>
  <circle cx="34" cy="30" r="2.1"/><path d="M34 30 L34 17"/>
  <path d="M34 5 L28 11 L31 14"/>{close}
 </g>{gold_core}
</svg>"""
...
.snty-gv-sigil {width:46px;height:46px;display:grid;place-items:center;flex:0 0 46px}
.snty-brand-mark{display:block;filter:drop-shadow(0 0 12px rgba(142,249,255,.14))}
@media (prefers-reduced-motion:no-preference){.snty-brand-mark{animation:snty-mark-breathe 8s ease-in-out infinite}}
@keyframes snty-mark-breathe{0%,100%{opacity:.92}50%{opacity:1;filter:drop-shadow(0 0 16px rgba(153,69,255,.20))}}
...
@media (max-width:600px){
 ...
 .snty-gv-sigil{width:40px;height:40px;flex:0 0 40px}
...
def _gv_header(db_path: str | Path, canonical_file: str | Path) -> None:
    ...
    st.markdown(
        ...
        f'<div class="snty-gv-sigil">{_snty_crystalline_mark_svg(44, mode not in ("DUAL · UNARMED", "LIVE · GATED"), "gv")}</div><div><div class="snty-gv-title">SENTINUITY SOVEREIGN OS</div>'
