...
def _render_research_activity_glassbox(query_db) -> None:
    """Operator-visible, provenance-first research stream."""
    df = _qdb(query_db, "SELECT created_at,event_type,actor,task_id,query,source_ref,licence,safety_status,summary,confidence,disposition FROM research_activity_ledger ORDER BY created_at DESC LIMIT 80")
    if df is None or df.empty:
        st.info("No structured research events yet. GitHub Scout must emit SEARCH_PLANNED/EXECUTED and SOURCE_FETCHED events.")
        return
    st.caption("Exact searches, sources, licence/safety state, extracted findings and disposition. Credentials are never displayed.")
    for _, row in df.head(30).iterrows():
        event=str(row.get('event_type') or 'EVENT'); actor=str(row.get('actor') or 'SYSTEM')
        query=str(row.get('query') or '').strip(); source=str(row.get('source_ref') or '').strip()
        summary=str(row.get('summary') or '').strip(); licence=str(row.get('licence') or 'NOT_RESOLVED')
        safety=str(row.get('safety_status') or 'NOT_RECORDED'); disposition=str(row.get('disposition') or '')
        title=f"{event} · {actor}"
        with st.expander(title, expanded=event in ('SEARCH_EXECUTED','INSIGHT_EXTRACTED')):
            if query: st.code(query, language=None)
            if source: st.write(f"Source: {source}")
            st.write(summary or "No summary recorded")
            st.caption(f"Licence: {licence} · Safety: {safety} · Disposition: {disposition}")
...
""", unsafe_allow_html=True)
    ...
    _safe_panel("Research glassbox", _render_research_activity_glassbox, query_db)
