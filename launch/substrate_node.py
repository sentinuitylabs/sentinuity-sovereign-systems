...
def _mark_or_none(*candidates: Any) -> Optional[float]:
    """First strictly-positive candidate price, else None.

    SIGNOFF_MARK_HONESTY_20260812. A mark is evidence, not a default. NULL,
    empty string, non-numeric and 0.0 all mean "no executable mark recorded"
    in these tables, and none of them may be silently promoted into a price.
    Callers must treat None as unknown and render it as unknown — never as
    flat, and never as -100%.
    """
    for v in candidates:
        if v is None:
            continue
...
def _render_macro_trade_book() -> None:
    ...
    if open_pos:
        for p in open_pos:
            ...
            # SIGNOFF_MARK_HONESTY_20260812: this row previously read
            #     cur = _num(current_price, _num(last_price, entry))
            #     pnl_pct = (cur - entry) / entry * 100
            # _num() only substitutes on None, so a stored 0.0 flowed straight
            # through and rendered exactly -100.00% whether the token had
            # actually gone to zero or the mark was simply missing; and a NULL
            # mark silently became `entry`, rendering 0.00% and hiding a real
            # rug. Neither state is a price. _mark_or_none() returns None for
            # both, and the row renders "NO MARK" instead of a coordinate.
            #
            # Note this lane reads substrate_positions / substrate_paper_positions,
            # NOT paper_positions, so services/position_truth_payload.py does not
            # apply here. The rule it encodes — never substitute — does.
            cur   = _mark_or_none(p.get("current_price"), p.get("last_price"))
            ...
            priced = cur is not None and entry > 0
            pnl_pct = ((cur - entry) / entry * 100) if priced else None
            ...
            # Unknown is grey. It is neither a gain nor a loss, and colouring it
            # as either would be an assertion the evidence does not support.
            cur_str = f"→ ${cur:.6g}" if priced else "→ NO MARK"
            pct_str = f"{pnl_pct:+.1f}%" if priced else "unpriced"
            if not priced:
                pnl_col = C_DIM
            st.markdown(
                ...
                f"<span style='color:{'#ccc' if priced else C_DIM};'>{cur_str}</span>"
                f"<span style='color:{pnl_col};margin-left:auto;'>{upnl:+.2f} USD ({pct_str})</span>"
