...
LOCK_BACKOFF_MIN = max(60, int(os.getenv("FORGE_LOCK_BACKOFF_MIN_SEC", "60")))
LOCK_BACKOFF_MAX = max(LOCK_BACKOFF_MIN, int(os.getenv("FORGE_LOCK_BACKOFF_MAX_SEC", "900")))
...
def run() -> None:
    ...
    while True:
        try:
            ...
            if _is_db_lock(exc):
                # Auxiliary Forge work yields to execution and price truth.
                # Never turn a transient lock into a multi-line traceback storm.
                log.warning("[FORGE_DB_LOCK_BACKOFF] sleep=%ss error=%s", lock_backoff, exc)
                time.sleep(lock_backoff)
                lock_backoff = min(LOCK_BACKOFF_MAX, lock_backoff * 2)
                continue

            log.error("Forge loop error: %s: %s", type(exc).__name__, exc)
            try:
